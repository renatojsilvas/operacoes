...
RUN RID="linux-$(test "$TARGETARCH" = amd64 && echo x64 || echo "$TARGETARCH")" && \
    echo "RID=$RID" && \
    dotnet restore src/Operacoes.API/Operacoes.API.csproj -r "$RID" -p:PublishReadyToRun=true
COPY src/ src/
RUN RID="linux-$(test "$TARGETARCH" = amd64 && echo x64 || echo "$TARGETARCH")" && \
    dotnet publish src/Operacoes.API/Operacoes.API.csproj -c Release -r "$RID" --self-contained false -p:PublishReadyToRun=true -o /app/publish --no-restore
...
EXPOSE 8080
ENTRYPOINT ["dotnet", "Operacoes.API.dll"]
